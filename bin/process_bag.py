...
def analyze_ros2_bag(bag_path, namespace, t0=0, tf=float('inf')):
    ...
    waypoints = np.array(
        data["/*/controller"]["ros__parameters"]["policy"]["waypoints"]).reshape(-1, 6)
    ...
    verts_all = np.einsum('ij,njk->nik', local_square, rotations) + \
        wp_pos[:, np.newaxis, :]  # shape (N, 4, 3)
    ...
    if not experiment_name:  # Handle root path case
        print(
            f"Warning: Cannot determine experiment name from path '{bag_path}'. Using 'unknown_experiment'.")
    ...
    if not resolved_bag_path.exists():
        ...
        print(
            f"Warning: Bag path '{bag_path}' does not exist. Guessing storage format based on name.")
        if bag_path.endswith('.mcap'):
            ...
            storage_id = 'sqlite3'
            bag_uri = bag_path  # Use the original path string as URI
        # Assume it's intended as a directory for sqlite3
        elif '.' not in Path(bag_path).name:
            storage_id = 'sqlite3'
            bag_uri = bag_path  # Use the original path string as URI
        else:
            print(
                f"Error: Cannot determine storage type for non-existent path with unknown extension: {bag_path}")
            sys.exit(1)
        ...
        if mcap_files:
            if len(mcap_files) > 1:
                print(
                    f"Warning: Multiple MCAP files found in {resolved_bag_path}. Using the first one: {mcap_files[0].name}")
            print(
                f"Found MCAP file: {mcap_files[0].name}. Setting storage_id='mcap' and URI to the file path.")
            storage_id = 'mcap'
            # CRITICAL: URI must be the MCAP file itself, resolved
            bag_uri = str(mcap_files[0].resolve())
        elif db3_files:
            # If db3 files exist, the URI should be the directory itself.
            print(
                f"Found db3 file(s): {[f.name for f in db3_files]}. Setting storage_id='sqlite3' and URI to the directory path.")
            storage_id = 'sqlite3'
            # URI is the directory for sqlite3
            ...
            print(
                f"Error: Could not determine storage type for directory {resolved_bag_path}. No .mcap, .db3, or metadata.yaml found.")
            sys.exit(1)
        ...
        if resolved_bag_path.suffix == '.mcap':
            ...
            print(
                f"Note: For .db3 file, using parent directory as URI: {bag_uri}")
        else:
            print(
                f"Error: Input path is a file with unrecognized suffix: {resolved_bag_path.suffix}")
            sys.exit(1)
    ...
    required_topics = [f"/{namespace}/observations", f"/{namespace}/odom",
                       f"/{namespace}/ctbr_cmd", f"/{namespace}/trajectory", "/ctbr_cmd"]
    missing_topics = []
    ...
    for req_topic in required_topics:
        ...
        if req_topic not in type_map and f"/{base_topic}" not in type_map:
            ...
            # Global topics like /ctbr_cmd
            elif not req_topic.startswith(f"/{namespace}/"):
    ...
    while reader.has_next():
        try:
            ...
            if topic not in type_map:
                ...
                print(
                    f"Warning: Skipping message from topic '{topic}' not initially listed in type map.")
                continue
            ...
            try:
                ...
                print(
                    f"Error deserializing message for topic {topic} ({msg_type}): {e}")
            ...
            if normalized_topic == odom_topic:
                ...
                if cf_name_in_msg is not None and namespace in cf_name_in_msg:
                    ...
                    thrust_pwm.append(
                        getattr(message, 'thrust_pwm', float('nan')))
                    thrust_N.append(getattr(message, 'thrust_n', float('nan')))
                    roll_rate.append(
                        getattr(message, 'roll_rate', float('nan')))
                    pitch_rate.append(
                        getattr(message, 'pitch_rate', float('nan')))
                ...
                # Default to NaN array if missing
                traj["x"].append(getattr(message, 'x', [float('nan')]*3))
                traj["x_dot"].append(
                    getattr(message, 'x_dot', [float('nan')]*3))
                traj["x_ddot"].append(
                    getattr(message, 'x_ddot', [float('nan')]*3))
                traj["x_dddot"].append(
                    getattr(message, 'x_dddot', [float('nan')]*3))
                traj["x_ddddot"].append(
                    getattr(message, 'x_ddddot', [float('nan')]*3))
                traj["yaw"].append(getattr(message, 'yaw', float('nan')))
                traj["yaw_dot"].append(
                    getattr(message, 'yaw_dot', float('nan')))
                traj["yaw_ddot"].append(
                    getattr(message, 'yaw_ddot', float('nan')))
                ...
                corners = getattr(message, 'corners_pos_b_curr',
                                  float('nan')).reshape(4, 3)
    ...
    print(
        f"Finished reading bag data. Read {message_count} messages total, processed {processed_count} within time range [{t0}, {tf}].")

    # -- Print initial pose ---
    quat = [gt_quat["x"][0], gt_quat["y"][0], gt_quat["z"][0], gt_quat["w"][0]]
    rpy = R.from_quat(quat).as_euler(
        'xyz', degrees=True)  # 'xyz' = roll, pitch, yaw

    print("Initial pose:")
    print(
        f"Position: x = {gt_pos['x'][0]:.3f}, y = {gt_pos['y'][0]:.3f}, z = {gt_pos['z'][0]:.3f}")
    print(
        f"Orientation (RPY): roll = {rpy[0]:.3f}°, pitch = {rpy[1]:.3f}°, yaw = {rpy[2]:.3f}°")
    ...
    if traj["x"]:
        ...
        if not traj["x"].size:
            print(
                "Warning: Trajectory data was found but resulted in empty arrays after processing.")
    ...
    if gt_quat["x"]:
        ...
        valid_quat_mask = np.all(np.isfinite(quaternions), axis=1) & (
            np.linalg.norm(quaternions, axis=1) > 1e-6)
        if not np.all(valid_quat_mask):
            ...
            try:
                if np.any(valid_quat_mask):  # Only convert if there are valid ones
                    ...
                    euler_angles[valid_quat_mask] = R.from_quat(
                        valid_quats).as_euler('xyz', degrees=True)
            except ValueError as e:
                print(
                    f"Error converting valid quaternions to Euler angles: {e}. Euler angles will contain NaNs.")
                # Keep euler_angles as initialized with NaNs
        else:
            # All quaternions are valid
            try:
                euler_angles = R.from_quat(
                    quaternions).as_euler('xyz', degrees=True)
            except ValueError as e:
                print(
                    f"Error converting quaternions to Euler angles: {e}. Filling with NaNs.")
                euler_angles = np.full((len(quaternions), 3), np.nan)
    ...
    fig_gt.tight_layout(rect=[0, 0.03, 1, 0.97])
    gt_filename = output_plot_dir / f"{namespace}_ground_truth"
    try:
        print(f"Saving ground truth plot to {gt_filename}")
        fig_gt.savefig(gt_filename.with_suffix('.eps'), format='eps',
                       bbox_inches='tight', dpi=300, facecolor=fig_gt.get_facecolor())
        fig_gt.savefig(gt_filename.with_suffix('.png'), format='png',
                       bbox_inches='tight', dpi=300, facecolor=fig_gt.get_facecolor())
    except Exception as e:
        print(f"Error saving ground truth plot: {e}")

    # --- Positions comparison plot ---
    # Check if traj data is valid 2D array
    if timestamps_traj and traj["x"].ndim == 2 and traj["x"].shape[0] > 0:
        ...
        axs_pos[0].plot(timestamps_traj, traj["x"][:, 0],
                        label="Desired", linestyle='--')
        axs_pos[0].set_ylabel("x [m]")
        axs_pos[0].legend()
        axs_pos[0].grid(True)

        axs_pos[1].plot(timestamps, gt_pos["y"], label="Actual")
        axs_pos[1].plot(timestamps_traj, traj["x"][:, 1],
                        label="Desired", linestyle='--')
        ...
        axs_pos[2].plot(timestamps_traj, traj["x"][:, 2],
                        label="Desired", linestyle='--')
        ...
        try:
            ...
            fig_pos.savefig(pos_filename.with_suffix('.eps'), format='eps',
                            bbox_inches='tight', dpi=300, facecolor=fig_pos.get_facecolor())
            fig_pos.savefig(pos_filename.with_suffix('.png'), format='png',
                            bbox_inches='tight', dpi=300, facecolor=fig_pos.get_facecolor())
        except Exception as e:
            print(f"Error saving position comparison plot: {e}")
    else:
        print(
            "Skipping position comparison plot (no valid trajectory data found/processed).")
    ...
    if timestamps_cmd:  # Check if there's command data to compare
        ...
        axs_rates[0].plot(timestamps_cmd, roll_rate,
                          label="Desired", linestyle='--')
        ...
        axs_rates[1].plot(timestamps_cmd, pitch_rate,
                          label="Desired", linestyle='--')
        ...
        axs_rates[2].plot(timestamps_cmd, yaw_rate,
                          label="Desired", linestyle='--')
        ...
        rates_filename = output_plot_dir / \
            f"{namespace}_angular_velocity_comparison"
        try:
            print(
                f"Saving angular velocity comparison plot to {rates_filename}")
            fig_rates.savefig(rates_filename.with_suffix(
                '.eps'), format='eps', bbox_inches='tight', dpi=300, facecolor=fig_rates.get_facecolor())
            fig_rates.savefig(rates_filename.with_suffix(
                '.png'), format='png', bbox_inches='tight', dpi=300, facecolor=fig_rates.get_facecolor())
        except Exception as e:
            print(f"Error saving angular velocity plot: {e}")
    else:
        print(
            "Skipping angular velocity comparison plot (no command data found/processed).")
    ...
    if timestamps_cmd:
        ...
        try:
            ...
            fig_ctbr.savefig(ctbr_filename.with_suffix('.eps'), format='eps',
                             bbox_inches='tight', dpi=300, facecolor=fig_ctbr.get_facecolor())
            fig_ctbr.savefig(ctbr_filename.with_suffix('.png'), format='png',
                             bbox_inches='tight', dpi=300, facecolor=fig_ctbr.get_facecolor())
        except Exception as e:
    ...
    if timestamps_obs:
        ...
        try:
            ...
            fig_obs.savefig(obs_filename.with_suffix('.eps'), format='eps',
                            bbox_inches='tight', dpi=300, facecolor=fig_obs.get_facecolor())
            fig_obs.savefig(obs_filename.with_suffix('.png'), format='png',
                            bbox_inches='tight', dpi=300, facecolor=fig_obs.get_facecolor())
        except Exception as e:
...
#   #   print("Skipping CTBR command plot (no command data found/processed).")
    ...
    if gt_pos["x"]:
        ax3d.plot(gt_pos["x"], gt_pos["y"], gt_pos["z"],
                  label="Actual Trajectory")

    # Plot desired trajectory if available and valid
    if timestamps_traj and traj["x"].ndim == 2 and traj["x"].shape[0] > 0:
        ax3d.plot(traj["x"][:, 0], traj["x"][:, 1], traj["x"][:, 2],
                  label="Desired Trajectory", linestyle='--', color='orange')

    # Plot red points at times_pass_gates
    if gt_pos["x"] and timestamps:
        ...
        for i, t_gate in enumerate(times_pass_gates):
            ...
            ax3d.scatter(x_array[idx], y_array[idx], z_array[idx], color='red', s=8, marker='o',
    ...
    for square in verts_all:
        poly = Poly3DCollection(
            [square], color=square_color, alpha=square_alpha, edgecolor='k')
    ...
    try:
        ...
        fig3d.savefig(traj3d_filename.with_suffix('.eps'), format='eps',
                      bbox_inches='tight', dpi=300, facecolor=fig3d.get_facecolor())
        fig3d.savefig(traj3d_filename.with_suffix('.png'), format='png',
                      bbox_inches='tight', dpi=300, facecolor=fig3d.get_facecolor())
    except Exception as e:
    ...
    if not timestamps or not gt_euler["roll"] or all(np.isnan(gt_euler["roll"])):
        ...
        def rotate_points(points, roll, pitch, yaw):
            ...
            try:
                ...
                print(
                    f"Warning: Skipping rotation due to invalid Euler angles for SciPy: roll={roll}, pitch={pitch}, yaw={yaw} ({e})")
        ...
        def update_animation(_):
            ...
            anim_ax.set_title(
                f"Drone Trajectory Animation ({namespace}) - Time: {timestamps[frame_idx]:.2f}s")
        ...
        if not all_x or not all_y or not all_z:
            print(
                "Error: No valid position data available for setting animation axes limits.")
        ...
        anim_ax.set_zlim(max(0, min_z - padding_z), max_z +
                         padding_z)  # Ensure z starts >= 0

        # Calculate a reasonable size for the drone marker based on overall trajectory size
        # Use max range as a scale factor
        drone_size = max(x_range, y_range, z_range)
        ...
        if timestamps_traj and traj["x"].ndim == 2 and traj["x"].shape[0] > 0:
            anim_ax.plot(traj["x"][:, 0], traj["x"][:, 1], traj["x"][:, 2],
                         label="Desired Traj.", linestyle='--', color='orange', alpha=0.7)

        # Plot the full actual trajectory statically and faintly
        anim_ax.plot(gt_pos["x"], gt_pos["y"], gt_pos["z"], 'b-',
                     linewidth=1, alpha=0.3, label="Full Actual Traj.")

        # Initialize animated elements (lines/markers)
        # Trail will grow, others will move/rotate
        trail, = anim_ax.plot([], [], [], 'b-', linewidth=1.5,
                              label="Actual Trail")  # Growing trail
        drone_x, = anim_ax.plot(
            [], [], [], 'r-', linewidth=3)  # Drone X marker
        drone_y, = anim_ax.plot(
            [], [], [], 'g-', linewidth=3)  # Drone Y marker
        current_pos_marker, = anim_ax.plot(
            [], [], [], 'ko', markersize=4, label='Current Pos')  # Current position dot
        drone_quiver = anim_ax.quiver(
            0, 0, 0, 0, 0, 0, color='red', length=0.2)
        ...
        if len(timestamps) > 1:
            ...
            interval_ms = max(1, int(median_dt * 1000))
            # Calculate FPS, cap at 30 for GIF
            fps = min(30, 1000.0 / interval_ms)
        else:
            interval_ms = 50  # Default interval for single frame
            fps = 20  # Default FPS

        print(
            f"Animation settings: interval={interval_ms}ms, target save fps={fps:.1f}")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print(
            "  [t0]:              Optional start time in seconds relative to the bag start (default: 0).")
        print(
            "  [tf]:              Optional end time in seconds relative to the bag start (default: inf).")
        ...
        if tf_arg <= t0_arg:
            print(
                f"Error: End time (tf={tf_arg}) must be greater than start time (t0={t0_arg}).")
            sys.exit(1)
